...
const ProjectUpdate = ({ isModalOpen, closeModal, projectId }) => {
  ...
  useEffect(() => {
    ...
    const fetchProjectData = async () => {
      try {
        const response = await axios.get(`https://localhost:7206/api/Project/${projectId}`);
        const project = response.data;
        setName(project.name);
        setDescription(project.description);
        setCategories(project.categories);
        setTechnologies(
          project.technologies.split(', ').map((tech) => technologiesOptions.find((t) => t.name === tech)).filter(Boolean)
        );
        // Note: We can't set files here as we can't retrieve file information from the server
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://localhost:7206/api/Project/${projectId}`, formData, {
        headers: {
    ...
    <>
      ...
      <form className="max-w-md mx-auto p-6 bg-gray-900 rounded-lg shadow-lg text-white" onSubmit={handleSubmit}>
            ...
            <div className="relative mt-1">
              <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-gray-800 border border-gray-700 rounded-md shadow-sm cursor-default focus:outline-none focus:ring-1 focus:ring-orange-500 focus:border-orange-500 sm:text-sm">
                <span className="block truncate">
                  {technologies.length === 0 ? 'Select Technologies' : technologies.map((t) => t.name).join(', ')}
                </span>
                ...
                <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-base bg-gray-800 border border-gray-700 rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                  {technologiesOptions.map((tech) => (
                    <Listbox.Option
                      key={tech.id}
                      className={({ active }) =>
                        `${active ? 'text-white bg-orange-600' : 'text-gray-300'} cursor-default select-none relative py-2 pl-10 pr-4`
                      }
                      value={tech}
                    >
                      {({ selected, active }) => (
                        <>
                          <span className={`${selected ? 'font-medium' : 'font-normal'} block truncate`}>{tech.name}</span>
                          {selected ? (
                            <span className={`${active ? 'text-white' : 'text-orange-600'} absolute inset-y-0 left-0 flex items-center pl-3`}>
        ...
        <div className="mb-4">
          ...
          <input type="file" multiple onChange={handleFileChange} className="w-full px-3 py-2 border border-gray-700 rounded bg-gray-800 text-white" />
        </div>
